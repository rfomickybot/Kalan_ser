...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('Kalan working ' + config.WORKTYPE + ' now 💙'));
          // thanks to afnanplk
	    if (config.LANG == 'EN' || config.LANG == 'ML') {
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(conn.user.jid, fs.readFileSync("./photo/amalser.png"), MessageType.image, { caption: `*ʜᴇʏ ʙʀᴏ ᴋᴀʟᴀɴsᴇʀ ᴡᴏʀᴋɪɴɢ ` + config.WORKTYPE + ` 🥂* \n\n*ᴛʜᴀɴᴋʏᴏᴜ ғᴏʀ ᴜsɪɴɢ ᴋᴀʟᴀɴsᴇʀ🥂*`});
                } else {
